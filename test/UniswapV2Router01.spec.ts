...
import { expandTo18Decimals, getApprovalDigest, mineBlock, MINIMUM_LIQUIDITY, resolveChallenge } from './shared/utilities'
...
describe('UniswapV2Router{01,02}', () => {
	const routerVersion = RouterVersion.UniswapV2Router02;
	...
	describe(routerVersion, () => {
		...
		describe('swapExactTokensForTokens', () => {
			...
			it('gas', async () => {
				...
				expect(receipt.gasUsed).to.eq(
					{
						[RouterVersion.UniswapV2Router02]: 102806
					}[routerVersion as RouterVersion]
				)
			}).retries(3)
		})
		...
		describe('swapExactETHForTokens', () => {
			...
			it('happy path', async () => {
				...
				const WETHPairToken0 = await WETHPair.token0()
				await expect(
					router.swapExactETHForTokens(0, [WETH.address, WETHPartner.address], wallet.address, MaxUint256, challengeKey, {
						...overrides,
						value: swapAmount
					})
				)
			...
			it('gas', async () => {
				...
				expect(receipt.gasUsed).to.eq(
					{
						[RouterVersion.UniswapV2Router02]: 139656
					}[routerVersion as RouterVersion]
				)
			}).retries(3)
		})
